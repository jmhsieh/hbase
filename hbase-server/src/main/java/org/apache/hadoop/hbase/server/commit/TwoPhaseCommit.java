 ...
 * implementation.
 * <p>
 * <tt>this</tt> also has an {@link ErrorCheckable} so it can monitor for errors to the running
 * operation. Periodically while running, the error monitor will be checked and the operation failed
 * with the exception thrown via {@link ErrorCheckable#failOnError()}. If an error is found,
 * {@link #cleanup(Exception)} is called with the found exception and then we call {@link #finish()}
 * and count down {@link #getCompletedLatch()}.
 * <p>
...
public abstract class TwoPhaseCommit<L extends TwoPhaseCommitErrorListener<E>, E extends Exception>
   ...
   *          used via {@link #run()} or {@link #call()}) until the 'allow commit' latch reaches
   *          zero.
